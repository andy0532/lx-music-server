...
export class ListDataManage {
      ...
      void this.snapshotDataManage
        ...
        .then(async (snapshotInfo) => {
          if (snapshotInfo.latest)
            listData = await this.snapshotDataManage.getSnapshot(
              snapshotInfo.latest,
            )
          if (!listData)
  ...
  getListData = async (): Promise<LX.Sync.List.ListData> => {
    return {
      ...
      userList: this.userLists.map((l) => ({
        ...l,
        list: this.allMusicList.get(l.id) ?? [],
      })),
    }
  }
  ...
  private readonly updateList = ({
    ...
    let targetList: LX.List.UserListInfo | undefined
    switch (id) {
  ...
  listDataOverwrite = async ({
    ...
    userList,
    tempList,
  }: MakeOptional<LX.List.ListDataFull, 'tempList'>): Promise<string[]> => {
    ...
    if (this.allMusicList.has(LIST_IDS.DEFAULT))
  ...
  userListCreate = async ({
    ...
    this.createUserList(
      { name, id, source, sourceListId, locationUpdateTime },
      position,
    )
  }
  ...
  userListsUpdatePosition = async (position: number, ids: string[]) => {
    ...
    newUserLists.splice(
      0,
      newUserLists.length,
      ...newUserLists.filter((mInfo) => map.has(mInfo.id)),
    )
    newUserLists.splice(
      Math.min(position, newUserLists.length),
      0,
      ...updateLists,
    )
  ...
  listMusicRemove = async (
    listId: string,
    ids: string[],
  ): Promise<string[]> => {
    const targetList = await this.getListMusics(listId)
    const listSet = new Set<string>()
    for (const item of targetList) listSet.add(item.id)
  ...
  listMusicMove = async (
    ...
    return [
      ...(await this.listMusicRemove(
        fromId,
        musicInfos.map((m) => m.id),
      )),
      ...(await this.listMusicAdd(toId, musicInfos, addMusicLocationType)),
    ]
  }

  listMusicUpdateInfo = async (
